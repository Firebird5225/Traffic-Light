...
void setup() {  // set up the digital I/O pins
  pinMode(eastEmr, INPUT);
  pinMode(westEmr, INPUT);
...
void loop() {
  ...
  if ((digitalRead(westButton) == HIGH) || forceWest || digitalRead(westEmr) == HIGH)  // request west>east traffic flow {
    if (trafficWest != true)
  ...
  if ((digitalRead(eastButton) == HIGH) || forceEast || digitalRead(eastEmr) == HIGH)  //when button is pressed starts the if statement
  {
    if (trafficWest == true) {
      trafficWest = false;
      delay(flowTime);
